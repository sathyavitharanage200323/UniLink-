...
public class AvailabilityController {
    ...
    /** Bulk upsert: replace all weekly recurring slots for a lecturer. */
    @PostMapping("/lecturer/{lecturerId}/weekly")
    public ResponseEntity<?> saveWeeklyAvailability(@PathVariable Long lecturerId,
                                                     @RequestBody List<AvailabilitySlotDTO> slots) {
        try {
            return ResponseEntity.ok(availabilityService.saveWeeklyAvailability(lecturerId, slots));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(Map.of("message", e.getMessage()));
        }
    }
