...
public interface AvailabilitySlotRepository extends JpaRepository<AvailabilitySlot, Long> {
    ...
    List<AvailabilitySlot> findByLecturerAndDayOfWeekOrderByStartTimeAsc(User lecturer, String dayOfWeek);

    @Modifying
    @Query("DELETE FROM AvailabilitySlot a WHERE a.lecturer = ?1 AND a.dayOfWeek IS NOT NULL")
    void deleteWeeklyByLecturer(User lecturer);
