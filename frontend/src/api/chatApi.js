...
export const cannedApi = {
  getByLecturer: (lecturerId) => api.get(`/canned-responses/lecturer/${lecturerId}`),
  create: (lecturerId, title, content) => api.post('/canned-responses', { lecturerId, title, content }),
  update: (id, title, content) => api.put(`/canned-responses/${id}`, { title, content }),
  delete: (id) => api.delete(`/canned-responses/${id}`),
};

export const disciplineApi = {
  getByStudent: (studentId) => api.get(`/discipline/student/${studentId}`),
  checkBlocked: (studentId, lecturerId) => api.get(`/discipline/check?studentId=${studentId}&lecturerId=${lecturerId}`),
  apply: (payload) => api.post('/discipline', payload),
  revoke: (id) => api.patch(`/discipline/${id}/revoke`),
};
...
export const userApi = {
  ...
  toggleDnd: (id, dnd, autoReplyMessage) => api.patch(`/users/${id}/dnd`, { dnd, autoReplyMessage }),
