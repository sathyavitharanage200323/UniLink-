    ...
    <div style={{
      ...
      <div style={{ textAlign: 'center', color: 'white' }}>
        ...
        <p style={{ opacity: 0.55, fontSize: '0.85rem', marginTop: 6 }}>
          Real-Time Chat Â· Appointments Â· Messages
        </p>
      </div>
...
export default function App() {
  const [activeUser, setActiveUser] = useState(() => {
    try { return JSON.parse(localStorage.getItem('unilink_user') || 'null'); }
    catch { return null; }
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const appts = activeUser.role === "STUDENT"
